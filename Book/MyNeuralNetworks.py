...
class Activation_Linear:
    ...
    def backward(self, dvalues):
        # 1 is derivative, 1 * dvalues = dvalues - the chain rule
        self.dinputs = dvalues.copy()
    ...
    def pre_update_params(self):
        if self.decay:
            self.current_learning_rate = \
                self.learning_rate * (1. / (1. + self.decay *
                                            self.iterations))
...
class Model:
    ...
    def finalize(self):
        ...
        for i in range(layer_count):
            ...
            # Also let's save aside the reference to
            # the last object with trainable parameters.
            # We'll need it to easily access bias and weights
            else:
                self.layers[i].prev = self.layers[i - 1]
    ...
    def backward(self, output, y):
        ...
        if self.softmax_classifier_output is not None:
            ...
            # but last which we already handled
    ...
    def save(self, path):
        ...
        # We do this so that we can save the model,
        # otherwise large datasets will exceed memory
